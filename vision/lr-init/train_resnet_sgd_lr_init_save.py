...
import utils.search_catapult_utils as search_catapult
...
def create_train_state(config: argparse.ArgumentParser, batch: Tuple):
    ...
    # create an optimizer
    opt = optax.inject_hyperparams(optax.sgd)(learning_rate = config.lr_zero, momentum = config.momentum)
...
def train_and_evaluate(config: argparse.ArgumentParser, train_ds: Tuple, test_ds: Tuple, sharpness_init = None):
    ...
    config.lr_zero = config.lr_trgt / config.warmup_steps
    print(f'lr zero: {config.lr_zero:0.8f}')
    
    ########### Estimate lr loss ######
    frwd_passes = 0
    batch = next(train_loader)

    if config.lr_zero < config.lr_trgt:
        lr_lower, lr_upper, frwd_passes = search_catapult.search_instability(config, state, batch, config.lr_zero)
        config.lr_init = lr_upper # let warmup induce catapult
    else:
        config.lr_init = config.lr_trgt
    ...
    for step in range(config.num_steps):  
        ...
        if step < config.warmup_steps:
            lr_step = schedules_utils.polynomial_warmup_save(state.step+1, config.lr_init, config.lr_trgt, config.warmup_steps, exponent = config.warmup_exponent) # state.step + 1 used because there is not training step yet
        else:
        ...
        result = np.array([state.step, epoch, lr_step, loss_step, accuracy_step, sharpness_init, config.lr_init, frwd_passes])
...
parser.add_argument('--lr_zero', type = float, default = 1e-04)
parser.add_argument('--eps', type = float, default = 0.1)
parser.add_argument('--lr_exp', type = float, default = 5.0)
...
while not divergence:
    ...
    divergence, train_results, eval_results, num_params, sharpness_init = train_and_evaluate(config, (x_train, y_train), (x_test, y_test), sharpness_init)

    # create a dataframe
    df_train = pd.DataFrame(train_results, columns = ['step', 'epoch', 'lr', 'loss_step', 'accuracy_step', 'sharpness_init', 'lr_init', 'frwd_passes'])
    df_train['num_params'] = num_params
    df_train['lr_init'] = config.lr_init
    # save training datai
    path = f'{save_dir}/train_save_{config.dataset}_{config.model}_scale{config.scale}_n{config.width}_w{config.widening_factor}_d{config.depth}_bias{config.use_bias}_{config.act_name}_I{config.init_seed}_J{config.sgd_seed}_{config.loss_name}_augment{config.augment}_{config.opt_name}_eps{config.eps}_lrexp{lr_exp:0.4f}_k{config.warmup_exponent}_p{config.decay_exponent}_Twarm{config.warmup_steps}_T{config.num_steps}_B{config.batch_size}_m{config.momentum}_{config.sharpness_method}.tab'    
    ...
    if not divergence:
        ...
        path = f'{save_dir}/eval_save_{config.dataset}_{config.model}_scale{config.scale}_n{config.width}_w{config.widening_factor}_d{config.depth}_bias{config.use_bias}_{config.act_name}_I{config.init_seed}_J{config.sgd_seed}_{config.loss_name}_augment{config.augment}_{config.opt_name}_eps{config.eps}_lrexp{lr_exp:0.4f}_k{config.warmup_exponent}_p{config.decay_exponent}_Twarm{config.warmup_steps}_T{config.num_steps}_B{config.batch_size}_m{config.momentum}_{config.sharpness_method}.tab'
