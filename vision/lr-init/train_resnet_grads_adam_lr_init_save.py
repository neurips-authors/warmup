...
def create_train_state(config: argparse.ArgumentParser, batch: Tuple):
    ...
    # create an optimizer
    opt = optax.inject_hyperparams(optim_utils.adam_init)(learning_rate = config.lr_zero, b1 = config.b1, b2 = config.b2)
...
parser.add_argument('--lr_zero', type = float, default = 0.0001)
...
while not divergence:
    ...
    train_path = f'{save_dir}/train_save_{config.dataset}_{config.model}_scale{config.scale}_n{config.width}_w{config.widening_factor}_d{config.depth}_bias{config.use_bias}_{config.act_name}_I{config.init_seed}_J{config.sgd_seed}_{config.loss_name}_augment{config.augment}_{config.opt_name}_eps{config.eps}_lr{config.lr_trgt:0.6f}_k{config.warmup_exponent}_p{config.decay_exponent}_Twarm{config.warmup_steps}_T{config.num_steps}_B{config.batch_size}_b1{config.b1}_b2{config.b2}.tab'

    eval_path = f'{save_dir}/eval_save_{config.dataset}_{config.model}_scale{config.scale}_n{config.width}_w{config.widening_factor}_d{config.depth}_bias{config.use_bias}_{config.act_name}_I{config.init_seed}_J{config.sgd_seed}_{config.loss_name}_augment{config.augment}_{config.opt_name}_eps{config.eps}_lr{config.lr_trgt:0.6f}_k{config.warmup_exponent}_p{config.decay_exponent}_Twarm{config.warmup_steps}_T{config.num_steps}_B{config.batch_size}_b1{config.b1}_b2{config.b2}.tab'
