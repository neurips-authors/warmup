...
@partial(jax.jit, static_argnums = 2)
def train_sharpness_lobpcg_step(state: TrainState, batch: Tuple, loss_function, vs, m_iter: int = 100, tol = 1e-09, dtype = jnp.float64):
    "Compute gradients, loss and accuracy for a single batch"
    x, y = batch
...
@partial(jax.jit, static_argnums = 2)
def train_pre_sharpness_lobpcg_step(state: TrainState, batch: Tuple, loss_function, vs, m_iter: int = 100, tol = 1e-09, dtype = jnp.float64):
    "Compute gradients, loss and accuracy for a single batch"
    x, y = batch
    x, y = jnp.asarray(x, dtype = dtype), jnp.asarray(y, dtype = dtype)
...
@partial(jax.jit, static_argnums = 2)
def train_pre_sharpness_lobpcg_custom_step(state: TrainState, batch: Tuple, loss_function, vs, m_iter: int = 100, tol = 1e-09, dtype = jnp.float64):
    "Compute gradients, loss and accuracy for a single batch"
...
@partial(jax.jit, static_argnums = 2)
def hessian_lobpcg_step(state: TrainState, batch: Tuple, loss_function, vs, m_iter = 100, tol = 1e-9, dtype = jnp.float64):
    """
