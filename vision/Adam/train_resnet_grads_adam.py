...
def train_and_evaluate(config: argparse.ArgumentParser, train_ds: Tuple, test_ds: Tuple):
    ...
    if config.opt_name == 'base_adam':
        grads_init = None
    if config.opt_name =='random_adam':
        flat_grads, rebuild_fn = jax.flatten_util.ravel_pytree(grads_init)
        ## estimate the mean and std of the grads
        ## mean = jnp.mean(flat_grads)
        ## std_dev = jnp.std(flat_grads)
        # estimate the norm of the gradient
        grad_norm = jnp.linalg.norm(flat_grads)
        # create a random array with the same mean and variance
        key = jax.random.PRNGKey(98)
        flat_rand = jax.random.normal(key, flat_grads.shape)
        flat_rand = flat_rand * grad_norm / jnp.linalg.norm(flat_rand)
        grads_init = rebuild_fn(flat_rand)
    # create an optimizer
...
parser.add_argument('--lr_start', type = float, default = 1e-05)
